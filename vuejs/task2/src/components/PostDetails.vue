<template>
  <div v-if="posts" :id='posts.id +"detail"' class="posts-comments-holder">

    <div id='details'> 
    <h3>Details of the Post</h3>
    <h5 class='body'>title: <span>{{posts.title}}</span></h5>
    <p class='desc'>Description: <span> {{posts.body}}</span></p>
    <hr>
    </div>

    <div id='comments'> 
  <div v-for="com in comments" :key="com.id" class='comments-username-body'>
      <p class='username'><img src="../../public/images/img_avatar.png" alt="Avatar" class="avatar">
      <span style="margin-left:10px">{{com.email}}</span></p>
      <p class='body'><span>{{com.name}}</span></p>
      <p class='body'><span>{{com.body}}</span></p>
      </div>
    </div>
    </div>
    <div v-else>
